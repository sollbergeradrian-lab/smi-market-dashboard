...
# Trend Detector
btc_change = float(crypto.loc[crypto.iloc[:,0] == "Bitcoin", "Tag %"].values[0])
oil_change = float(commodities.loc[commodities.iloc[:,0] == "Oil", "Tag %"].values[0])
gold_change = float(commodities.loc[commodities.iloc[:,0] == "Gold", "Tag %"].values[0])
...
for _, row in crypto.iterrows():
    ...
    name = row.iloc[0]
    price_usd = float(row["Preis"])
    price_eur = price_usd * usd_to_eur
    change = float(row["Tag %"])

    symbol = "+" if change > 0 else "-"

    report += f"- {name} {price_eur:.2f} EUR ({symbol}{change:.2f}%)\n"
...
# Market Signals
report += "\n## Market Signals\n"
report += f"- Bitcoin Trend: {btc_trend}\n"
report += f"- Oil Momentum: {oil_trend}\n"
report += f"- Gold Trend: {gold_trend}\n"
